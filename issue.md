...
- [x] **Step 1: Inisialisasi Project, Setup Environment (Dev vs Prod), & Docker Base** *(Selesai - Siap Review via PR)*
...
- **GitHub Issue**: [#1](https://github.com/rickyriskiawan/khazprokhir/issues/1)
- **Branch**: `feat/step-1-init-project-and-env`
- **Status**: Siap Direview (PR Pending)
...
| **Eksekusi Backend** | Berjalan dengan watch mode (misal `nodemon` atau `--watch`). | Berjalan via process manager (PM2 / containerized entrypoint) dengan auto-restart. |

#### C. Inisialisasi Server & Client
1. **Server (`server/`)**:
   - Inisialisasi `package.json` dengan konfigurasi ES Modules.
   - Install dependencies utama: `express`, `cors`, `dotenv`, `zod`, `@prisma/client`.
   - Install dev dependencies: `prisma`, `nodemon`.
   - Buat template `.env.example`, `.env.development`, dan template panduan `.env.production.example`.
2. **Client (`client/`)**:
   ...
   - Buat template konfigurasi `.env.development` (`VITE_API_URL=http://localhost:5000/api`) dan `.env.production.example`.
...
- [x] Struktur folder `client/` dan `server/` terbentuk rapi tanpa file duplikat.
- [x] Perintah `docker compose up -d` (atau Docker service) berhasil menjalankan container PostgreSQL untuk database lokal development.
- [x] Server dapat dijalankan dalam mode development dan merespons health check endpoint (`GET /api/health` mengembalikan status 200 OK).
- [x] Client Vite dapat dijalankan dalam mode development dan menampilkan halaman dasar tanpa error.
- [x] Konfigurasi file environment terpisah secara eksplisit antara `.env.development` dan petunjuk `.env.production.example`.
